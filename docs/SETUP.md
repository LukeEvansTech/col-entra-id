...
See [PERMISSIONS.md](PERMISSIONS.md) for details on required permissions.
...
> **Note**: Select **Runtime version: 7.2** for each module.
...
- `LicensesToInclude` - License types to process
- `WhatIf` - Set to `$false` for production runs

See [PARAMETERS.md](PARAMETERS.md) for full parameter reference.
