<#
.SYNOPSIS
    Azure Automation runbook to identify and optionally disable or soft delete inactive member users in an Entra ID/M365 tenant.
.DESCRIPTION
    This Azure Automation runbook connects to Microsoft Graph using a managed identity and identifies member users who:
    ...
    Inactive users can be disabled or soft deleted based on the supplied action parameter. The script supports WhatIf mode for safe testing.
...
.PARAMETER UserAction
    Specifies the action to take on inactive users. Valid values: SoftDelete, Disable. Default is Disable.
.PARAMETER WhatIf
...
.NOTES
    ...
    - User.ReadWrite.All (required for disable and soft delete operations)
...
param (
    ...
    [ValidateSet("SoftDelete", "Disable")]
...
function Process-InactiveMemberUsers {
    ...
    $errorCount = 0
    $errors = @()
    $actionVerb = if ($Action -eq "SoftDelete") { "soft delete" } else { "disable" }
    $actionPast = if ($Action -eq "SoftDelete") { "soft deleted" } else { "disabled" }

    Write-AutomationLog "Processing $($InactiveUsers.Count) inactive member users for $actionVerb..." "Information"

    if ($WhatIf) {
        Write-AutomationLog "WhatIf mode enabled - no $actionVerb operations will be performed" "Information"
    }

    foreach ($user in $InactiveUsers) {
        try {
            if ($WhatIf) {
                Write-AutomationLog "WhatIf: Would $actionVerb user '$($user.DisplayName)' ($($user.UserPrincipalName)) - Created: $($user.CreatedDate), Last sign-in: $($user.LastSignIn)" "Information"
                $processedCount++
                continue
            }

            if ($Action -eq "SoftDelete") {
            ...
            Write-AutomationLog "Successfully $actionPast user '$($user.DisplayName)' ($($user.UserPrincipalName)) - Created: $($user.CreatedDate), Last sign-in: $($user.LastSignIn)" "Information"
            $processedCount++
        }
        catch {
            $errorMessage = "Failed to $actionVerb user '$($user.DisplayName)' ($($user.UserPrincipalName)): $($_.Exception.Message)"
    ...
    Write-AutomationLog "$actionVerb operation complete: $processedCount users $actionPast, $errorCount errors" "Information"
...
Write-AutomationLog "Starting Inactive Member Users runbook (Azure Automation)" "Information"
Write-AutomationLog "User action: $UserAction" "Information"
...
Write-AutomationLog "Retrieving active member users with sign-in activity..." "Information"
try {
    $allMemberUsers = Get-MgUser -Filter "userType eq 'Member' and accountEnabled eq true" -All -Property "id,userPrincipalName,displayName,mail,userType,signInActivity,accountEnabled,assignedLicenses,createdDateTime,department" -ConsistencyLevel "eventual"
    Write-AutomationLog "Retrieved $($allMemberUsers.Count) active member users" "Information"
    ...
    try {
        $allMemberUsers = Get-MgUser -All -Property "id,userPrincipalName,displayName,mail,userType,signInActivity,accountEnabled,assignedLicenses,createdDateTime,department" -ConsistencyLevel "eventual" | Where-Object { $_.UserType -eq 'Member' -and $_.AccountEnabled -eq $true -and $_.AssignedLicenses -and $_.AssignedLicenses.Count -gt 0 }
        Write-AutomationLog "Retrieved $($allMemberUsers.Count) active member users via alternative approach" "Information"
...
Write-AutomationLog "Found $inactiveCount inactive member users (inactive >= $InactiveDays days)" "Information"
...
if ($inactiveCount -gt 0) {
    ...
    $result = Process-InactiveMemberUsers -InactiveUsers $inactiveUsers -Action $UserAction -WhatIf:$WhatIf
    ...
    Write-AutomationLog "No inactive member users found - no action required" "Information"
}
